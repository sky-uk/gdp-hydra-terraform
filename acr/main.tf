...
resource "azuread_application" "acr_application" {
...
resource "azuread_service_principal" "acr_service_principal" {
  application_id = "${azuread_application.acr_application.application_id}"
}
...
resource "random_string" "acr_password" {
  ...
  keepers = {
    service_principal = "${azuread_service_principal.acr_service_principal.id}"
  }
}

resource "azuread_service_principal_password" "acr_service_principal_password" {
  service_principal_id = "${azuread_service_principal.acr_service_principal.id}"
...
resource "azurerm_role_assignment" "acr_service_principal_role" {
  ...
  principal_id         = "${azuread_service_principal.acr_service_principal.id}"
}
