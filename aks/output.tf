...
output "cluster_client_certificate" {
  ...
  value       = "${base64decode(azurerm_kubernetes_cluster.aks.0.kube_config.0.client_certificate)}"
  sensitive   = true
}

output "cluster_client_key" {
  description = "The client key used for connecting to the cluster"
  value       = "${base64decode(azurerm_kubernetes_cluster.aks.0.kube_config.0.client_key)}"
  sensitive   = true
}
